...
export const user = mysqlTable('user', {
  ...
  userType: varchar("user_type", { length: 20 }).default("customer"), // customer, driver, admin, vendor
...
// Subscriptions - for managing subscription plans
export const subscriptions = mysqlTable("subscriptions", {
  ...
  // Billing cycle
  billingCycle: varchar("billing_cycle", { length: 20 }).notNull().default("monthly"), // daily, weekly, monthly, yearly, custom
  billingIntervalCount: int("billing_interval_count").default(1), // e.g., 2 months, 3 weeks
  
  // Duration
  durationDays: int("duration_days"), // Total duration in days (null for unlimited)
  expiresAfterDays: int("expires_after_days"), // Auto-expire after X days from purchase
  
  // Trial period
  trialDays: int("trial_days").default(0), // Free trial period in days
  
  // Features and limits
  features: json("features"), // Array of feature strings
  maxUsers: int("max_users"), // Maximum number of users (null for unlimited)
  maxOrders: int("max_orders"), // Maximum number of orders (null for unlimited)
  maxProducts: int("max_products"), // Maximum number of products (null for unlimited)
  
  // Discount and promotions
  discountPercentage: decimal("discount_percentage", { precision: 5, scale: 2 }).default('0.00'), // Discount percentage
  comparePrice: decimal("compare_price", { precision: 10, scale: 2 }), // Original price before discount
...
// User Subscriptions - tracks user subscription purchases
export const userSubscriptions = mysqlTable("user_subscriptions", {
  id: varchar("id", { length: 255 }).primaryKey(),
  userId: varchar("user_id", { length: 255 }).notNull(),
  subscriptionId: varchar("subscription_id", { length: 255 }).notNull(),
  orderId: varchar("order_id", { length: 255 }), // Reference to order if purchased
  
  // Subscription details
  subscriptionName: varchar("subscription_name", { length: 255 }).notNull(), // Snapshot at time of purchase
  price: decimal("price", { precision: 10, scale: 2 }).notNull(), // Price paid at time of purchase
  currency: varchar("currency", { length: 3 }).default("PKR"),
  ...
  expiryDate: datetime("expiry_date"), // When subscription expires (null for lifetime)
  cancelledAt: datetime("cancelled_at"),
  
  // Billing
  nextBillingDate: datetime("next_billing_date"), // For recurring subscriptions
  lastBillingDate: datetime("last_billing_date"),
  billingCycle: varchar("billing_cycle", { length: 20 }), // monthly, yearly, etc.
...
export const userSubscriptionsRelations = relations(userSubscriptions, ({ one }) => ({
  ...
  order: one(orders, {
    fields: [userSubscriptions.orderId],
    references: [orders.id],
  }),
}));

// Vendor Profiles - Extended company information for vendor users
export const vendorProfiles = mysqlTable("vendor_profiles", {
  id: varchar("id", { length: 255 }).primaryKey(),
  userId: varchar("user_id", { length: 255 }).notNull().unique(), // Reference to user table with userType='vendor'
  ...
  taxId: varchar("tax_id", { length: 100 }), // NTN or Sales Tax Number
  
  // Business Details
  businessType: varchar("business_type", { length: 50 }), // manufacturer, distributor, retailer, wholesaler
  industryCategory: varchar("industry_category", { length: 100 }),
  yearEstablished: int("year_established"),
  numberOfEmployees: varchar("number_of_employees", { length: 50 }), // 1-10, 11-50, 51-200, 201-500, 500+
  ...
  businessLicense: varchar("business_license", { length: 500 }), // Business license document URL
  taxCertificate: varchar("tax_certificate", { length: 500 }), // Tax registration certificate URL
  
  // Description & Social
  description: text("description"), // Company description
  shortBio: varchar("short_bio", { length: 500 }), // Short company bio
  ...
  // Commission & Settings
  commissionRate: decimal("commission_rate", { precision: 5, scale: 2 }).default('0.00'), // Percentage commission
  ...
  // Additional Info
  notes: text("notes"), // Admin notes about the vendor
